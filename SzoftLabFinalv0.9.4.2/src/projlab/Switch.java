...
/**
 * A váltókat megvalósító osztály
 */
public class Switch extends Tile {
	...
	/**
	 * A switch konstruktora, megkapja paraméterként az aktív irányt **csak
	 * teszteléshez**
	 *
	 * @param activeDir
	 */
		...
		if (train != null) {
			if (train.getPrevPos() == getDirA() && activeDir != null && getActiveDir().isFree()) {
				train.moveTrain(getActiveDir());
					...
					&& getDirB() == getActiveDir()) {
				train.moveTrain(getDirA());
			} else if (train.getPrevPos() == getDirB() && getDirA() != null && getDirA().isFree()
					&& getDirC() == getActiveDir()) {
				train.moveTrain(getDirC());
			} else if (train.getPrevPos() == getDirC() && getDirA() != null && getDirA().isFree()
					&& getDirB() == getActiveDir()) {
